...
class PlayState extends MusicBeatState
{
	...
	// soulless bg
	var blood:BGSprite;
	var shadowTheFuckingHedgehog:BGSprite;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			var backTrees:BGSprite = new BGSprite('PolishedP1/BackTrees', -600, -200, 0.7, 0.7);
			...
			var deadGuys:BGSprite = new BGSprite('PolishedP1/DeadGuys', -600, -200, 1, 1);
			deadGuys.setGraphicSize(Std.int(deadGuys.width * 1.4));
			deadGuys.isGore = true;
			add(deadGuys);
			...
			case 'TDP2':
				var bg:BGSprite = new BGSprite('soulless/bg', 800, 660, 0.75, 0.75);
				bg.setGraphicSize(Std.int(bg.width * 1.5));
				add(bg);

				blood = new BGSprite('soulless/blod', 800, 660, 0.75, 0.75);
				blood.setGraphicSize(Std.int(blood.width * 1.5));
				blood.visible = false;
				add(blood);

				shadowTheFuckingHedgehog = new BGSprite('soulless/shadow', 650, 450, 0.75, 0.75);
				shadowTheFuckingHedgehog.blend = ALPHA;
				shadowTheFuckingHedgehog.setGraphicSize(Std.int(shadowTheFuckingHedgehog.width * 1.35));
				shadowTheFuckingHedgehog.visible = false;
					...
					if (floorSus > 0)
					{
						for (susNote in 0...floorSus + 1)
						{
							...
							var sustainNote:Note = new Note(daStrumTime
								...
								+ (Conductor.stepCrochet / FlxMath.roundDecimal(songSpeed * speedMult, 2)),
	...
	public function endSong():Void
	{
		...
		if (CharSongList.songToFreeplayChar.exists(SONG.song.toLowerCase())
			&& !CharSongList.charactersUnlocked.contains(CharSongList.songToFreeplayChar.get(SONG.song.toLowerCase())))
		{
			CharSongList.charactersUnlocked.push(CharSongList.songToFreeplayChar.get(SONG.song.toLowerCase()));
			CharSongList.save();
		}
		...
		if (curStage == 'TDP2')
		{
			switch (curStep)
			{
				...
				case 260:
					defaultCamZoom = 0.8;
				case 546:
					FlxG.camera.flash(FlxColor.RED, 0.75);
					health = 0.5;

					for (obj in [blood, shadowTheFuckingHedgehog])
						obj.visible = true;
			}
		}
		...
		if (SONG.song.toLowerCase() == 'my-horizon')
		{
			switch (curStep)
			{
				...
				case 924:
					...
					FlxTween.tween(whiteFuck, {alpha: 1}, 6, {
						ease: FlxEase.cubeInOut,
						onComplete: function(twn:FlxTween)
						{
				...
				case 1824:
					...
					new FlxTimer().start(0, function(ok:FlxTimer)
					{
						...
						new FlxTimer().start(0,
							function(lol:FlxTimer) // xd lmafo never thought i would use while loops but here we go, EDIT: PLEASE DO NOT, IT WILL MAKE YOUR GAME FUCKING FREEZE
							{
								if (curBeat >= slashframe)
								{
									...
									trace("woah slash");
									if (FlxG.sound.music.time <= FlxG.sound.music.length - 8000)
										ok.reset();
								}
		...
		if (SONG.song.toLowerCase() == 'prey')
		{
			switch (curStep)
			{
				...
				case 3364:
					...
					gotcha.flipX = true;
					add(gotcha);
					FlxTween.tween(gotcha, {x: boyfriend.x + 500}, 0.2, {
						onComplete: function(yes:FlxTween)
						{
							remove(gotcha);
						}
					});
		...
		if (SONG.song.toLowerCase() == 'malediction')
		{
			switch (curStep)
			{
				...
				case 736:
					...
					if (curseStatic != null)
						curseStatic.visible = true;
				case 991:
					if (curseStatic != null)
					{
						FlxTween.tween(curseStatic, {alpha: 0}, 1, {
							ease: FlxEase.cubeInOut,
							onComplete: function(twn:FlxTween)
							{
								curseStatic.visible = false;
	...
	function literallyMyHorizon()
	{
		...
		fucklesBeats = false;
		fucklesDeluxe();
		FlxTween.tween(whiteFuck, {alpha: 0}, 1.5, {
			ease: FlxEase.cubeInOut,
			onComplete: function(twn:FlxTween)
			{
				remove(whiteFuck);
	...
	function doPopup(type:Int)
	{
		...
		var popuppos:Array<Int> = [
			errorRandom.int(0, Std.int(FlxG.width - popup.width)),
			errorRandom.int(0, Std.int(FlxG.height - popup.height))
		];
		popup.x = popuppos[0];
		popup.y = popuppos[1];
	...
	function spookyJumpscareAAA(char:String)
	{
		switch (char)
		{
			...
			case "knux":
				trace('SIMPLE JUMPSCARE');
				var doP3JumpKNUX:FlxSprite = new FlxSprite()
					.loadGraphic(Paths.image('JUMPSCARES/Knuckles' + (ClientPrefs.gore == false ? "NoGore" : ""), 'exe'));
	...
	function writeLyrics(text:String, duration:Float, color:FlxColor)
	{
		if (lyricText != null)
		{
			...
			FlxTween.tween(old, {alpha: 0}, 0.2, {
				onComplete: function(twn:FlxTween)
				{
					remove(old);
					old.destroy();
		...
		if (text.trim() != '' && duration > 0 && color.alphaFloat > 0)
		{
			...
			add(lyricText);
			lyricTween = FlxTween.tween(lyricText, {alpha: color.alphaFloat}, 0.2, {
				onComplete: function(twn:FlxTween)
				{
					trace("done");
					lyricTween = FlxTween.tween(lyricText, {alpha: 0}, 0.2, {
						startDelay: duration,
						onComplete: function(twn:FlxTween)
						{
							remove(lyricText);
