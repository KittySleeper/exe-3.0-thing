...
class FlxInputText extends FlxText
{
			...
			BackgroundColor:Int = FlxColor.WHITE, EmbeddedFont:Bool = true)
	{
		...
		caretIndex = 0;
		hasFocus = false;
		_charBoundaries = []; // Initialize to avoid null reference errors
		if (background)
		{
	...
	*/
	...
	override public function draw():Void
	{
		...
		if (hasFocus && capsLockEnabled)
    	{
        	var capsIndicator = new FlxText(x + width + 5, y - 5, 20, "CAPS");
        	capsIndicator.color = FlxColor.RED;
        	capsIndicator.scrollFactor = scrollFactor;
        	capsIndicator.cameras = cameras;
        	capsIndicator.draw();
    	}
	}
	...
	private function onKeyDown(e:KeyboardEvent):Void
	{
		...
		#if openfl
		if (Reflect.hasField(e, "capsLock"))
		{
			capsLockOn = Reflect.field(e, "capsLock");
		}
		#end
		...
		if (hasFocus)
		{
			...
			else if (key == 37)
			{
				...
				// Move text left if Ctrl is held
				else if (e.ctrlKey)
				{
					if (text.length > 0)
					{
						text = text.substr(1) + text.charAt(0);
			...
			else if (key == 39)
			{
				...
				// Move text right if Ctrl is held
				else if (e.ctrlKey)
				{
					if (text.length > 0)
					{
						text = text.charAt(text.length - 1) + text.substr(0, text.length - 1);
						caretIndex = caretIndex < text.length ? caretIndex + 1 : text.length;
					}
				}
			}
			// End key
    		...
    		{
        		var char:String = (e.charCode > 0) ? String.fromCharCode(e.charCode) : "";

				if (char == "" && key == 220) // backslash and el pipe (|)
    			{
        			char = e.shiftKey ? "|" : "\\";
    			}
        
        		if (capsLockEnabled && forceCase == ALL_CASES)
        		{
            		if (e.shiftKey) 
            		{
                		char = char.toLowerCase(); // Shift + Caps Lock = lower case
            		}
            		else
            		{
                		char = char.toUpperCase(); // Only Caps Lock = Upper case Boom
            		}
        		}
        
       	 		char = filter(char);
        		if (char.length > 0 && (maxLength == 0 || text.length < maxLength))
        		{
	...
	/**
	 * Handles keyup events (required for some platforms to properly process input).
	 */
	...
	{
		// Only apply forceCase & Caps Lock if it's not ALL_CASES
		if (!capsLockEnabled || forceCase != ALL_CASES)
    	{
		...
		// If forceCase == ALL_CASES, do not modify the case
